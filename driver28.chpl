...
//
  ...
  var tracerShortNames: [1..nt:int] string = ["PO4", "NO3", "SiO3", "NH4", "Fe", "Lig", "O2", "DIC", "DIC_ALT_CO2",
  ...
  marblWrapper.setSurfaceFluxForcingValues(
    ...
    temperature[colIdx,1], 10,0, 10.0, atm_co2[colIdx], atm_alt_co2[colIdx], 
    dust_flux[colIdx], iron_flux[colIdx], nox_flux[colIdx], nhy_flux[colIdx]);
  
  marblWrapper.surfaceFluxCompute(columnTracers, dt);

  marblWrapper.setInteriorTendencyForcingValues(zt, nt, columnTracers, dust_flux[colIdx], 1.0, 1.0, 1.0, temperature[colIdx,..], salinity[colIdx,..], zw);

  marblWrapper.interiorTendencyCompute(columnTracers, dt);

  var checkPath = "call_compute_subroutines.history.nc";

  for i in tracerShortNames.domain {
    ...
    writeln(name, " comparison: ", (+ reduce columnTracers) == (+ reduce expectedNewConcentration[colIdx,..]));
  }
  return;
}