module Marbl {
  /* 
    This module contains code for interoperating with the Fortran 90 MARBL biogeochemical library. The Fortran library's repository is found here: https://github.com/marbl-ecosys/MARBL/tree/marbl0.45.0
    ...
    MARBL interoperability works using three layers: One Chapel, one C, one Fortran. First, there is the Chapel layer, found in this file, that contains the `marblInteropType` record and a collection of `extern` procedures. The `marblInteropType` record is the main way users will interact with the MARBL library, using the methods of the record. Second, there is the C header layer, which connects the Chapel `extern` procedures to their definitions in Fortran. Finally, there is the Fortran layer, which defines the various `extern` procedures declared in the Chapel layer. 
  */

  use CTypes;
  require "MarblChapel.h";

  // Used to ensure only one marblInteropType has access to the settings files at a time, as
  // Fortran does not support concurrent reads to the same file
  var initBarrier: sync bool = true;


  /* 
    The main driver of MARBL interoperability. Supports shared- and distributed-memory parallel implementations, with each thread getting its own instance to compute multiple columns at once. 
   */
  extern record marblInteropType {
    ...
    /*
      Initializes the Chapel-side interop object, then creates and connects it to a Fortran-side MARBL instance. This method does NOT populate the Fortran-side MARBL instance with any information. To do that, use `initMarblInstance`.
     */
    proc init() {
      init this;
    ...
    /*
      ...
      :arg deltaZ: The thicknesses of the layers in the column. For example, if the boundaries of the first three layers are at depths 10, 20, and 22, the first three values of `deltaZ` should be 10, 10, and 2. 

      :arg zw: The depth from the surface to the bottom of each layer in the column. For example, if the boundaries of the first three layers are at depths 10, 20, and 22, the first three values of `zw` should be 10, 20, and 22. 

      :arg zt: The depth from the surface to the midpoints of each layer in the column. For example, if the boundaries of the first three layers are at depth 10, 20, and 22, the first three values of `zt` should be 5, 15, and 21. 
    ...
    /*
      ...
      :arg variableName: The name of the surface flux forcing value to set. Accepted values are
    ...
    proc setSurfaceFluxForcingValue(const ref variableName, const ref value) {
      set_surface_flux_forcing_value(this, variableName.c_str(), variableName.size: c_int, value: c_double);
    }

    /* 
      Copies the tracer values at the surface into the Fortran-side MARBL instance.
    ...
    /* 
      Computes the surface fluxes and updates the tracer array with the calculated tendencies.
    ...
    proc surfaceFluxCompute(ref tracerArray, dt) {
      ...
      var nt, nz: int;
      (nt,nz) = tracerArray.shape;
      update_surface_fluxes(this, nt: c_int, nz: c_int, c_ptrTo(tracerArray), dt: c_double);
    }
