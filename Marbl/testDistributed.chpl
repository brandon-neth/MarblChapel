// Full test of MARBL interop using distributed parallelism
...
coforall loc in Locales do on loc {
  // get local versions of important variables
  var nz = readDim(ncPath, "zt"): int;
  var nt = 32;
  var ztCol = readVar(ncPath, "zt", c_double, 1);
  var activeLevelCount = readVar(ncPath, "active_level_cnt", c_int, 1);
  var dust_flux = readVar(ncPath, "dust_flux", c_double, 1);
  ...
  forall colIdx_ in myTracerDomain.dim[0] {
    ...
    marblWrapper.initMarblInstance(nz, numParSubcols, numElementsSurfaceFlux, delta_z, zw, ztCol, activeLevelCount[colIdx]);
