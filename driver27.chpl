...
const surfaceDomain = {1..26,10..26};

/*
writeln("Surface domain: ", surfaceDomain);
writeln("Temperature domain: ", temperature.domain);
writeln("pco2 domain: ", pco2_air.domain);
writeln("H domain: ", H.domain);
writeln("z_r domain: ", z_r.domain);
*/
var tracerChanges: [tracerArray.dim[0],tracerArray.dim[1], 1..32] bool;
forall (x,y) in surfaceDomain {
  ...
  var columnSums: [tracerArray.dim[2]] (string,real);
  for t in 0..<columnTracers.shape[0] {
    columnSums[t+1] = (varNames[t], + reduce columnTracers[t+1,..]);
  }

  var columnH: [0..<Nz] real;
  var columnZr: [0..<Nz] real;
  var columnZw: [0..Nz] real;

  for i in columnH.domain {
    columnH[i] = H[i,x,y];
    columnZr[i] = z_r[i,x,y];
  }
  for i in columnZw.domain {
    columnZw[i] = z_w[i,x,y];
  }

  var marblWrapper: marbl_interop_type;
  marblWrapper.initMarblInstance(Nz, 1, 1,columnH, columnZw, columnZr);

  
  marblWrapper.setSurfaceFluxForcingValues(Nz, columnTracers.shape[0], columnTracers, salinity[x,y,1], 
                                           temperature[x,y,1], 10,0, 10.0, pco2_air[1,x,y], pco2_air_alt[1,x,y], 
                                           dust[1,x,y], fesedflux[1,x,y], nox[1,x,y], nhy[1,x,y]);
  
  marblWrapper.surfaceFluxCompute(columnTracers, 1.0);


  marblWrapper.setInteriorTendencyForcingValues(Nz, columnTracers.shape[0], columnTracers, dust[1,1,1], 1.0, 1.0, 1.0, temperature[x,y,..], salinity[x,y,..], columnZr);

  marblWrapper.interiorTendencyCompute(columnTracers, 1.0);
  for t in 1..32 {
    tracerChanges[x,y,t] = (+ reduce columnTracers(t,..)) !=  (+ reduce originalTracers(t,..));
  }
}

s.stop();
writeln("Elapsed time: ", s.elapsed());

  for t in 1..32 {
    var changeCount = + reduce tracerChanges[..,..,t] : int;
  writeln("Change count ", " ", t, ": ", changeCount);
  }
  
  //writeln("Changes per column: ", changeCount / (tracerChanges.shape[0] * tracerChanges.shape[1]));
  //writeln("Not changes per column: ", (tracerChanges.size - changeCount) / (tracerChanges.shape[0] * tracerChanges.shape[1]));
}