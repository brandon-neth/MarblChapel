...
//
 ...
 // writeln("Reading scalar values...");
  ...
  marblWrapper.setSurfaceFluxForcingValues(
    ...
    temperature[colIdx,1], 10,10, 1.0, atm_co2[colIdx], atm_alt_co2[colIdx], 
  ...
  for i in 2..2 {
    ...
    var expectedNewConcentration = originalConcentration + expectedTendency * dt;
    /*writeln(name);
    writeln("Original concentration sample value: ", originalConcentration[colIdx, 8]);
    writeln("Expected tendency: ", expectedTendency[colIdx,..]);
    writeln("Read-in concentration sample value: ", columnTracers_[i, 8]);
    writeln("Concentration value after computation: ", columnTracers[i,8]);
    writeln("Difference through computation? ", columnTracers_[i,8] != columnTracers[i,8]);
    writeln();*/
    //, " comparison: ", (+ reduce columnTracers) == (+ reduce expectedNewConcentration[colIdx,..]));
  }
  return;
}