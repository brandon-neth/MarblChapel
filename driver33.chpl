...
module MarblInterop {
  ...
  extern record marbl_interop_type {
    ...
    proc initMarblInstance(const ref gcm_num_levels, const ref gcm_num_PAR_subcols, const ref gcm_num_elements_surface_flux, ref gcm_delta_z, ref gcm_zw, ref gcm_zt, const ref active_level_cnt) {
      var lock = initBarrier.readFE();
      init_marbl_instance(this, gcm_num_levels: c_int, gcm_num_PAR_subcols: c_int, gcm_num_elements_surface_flux: c_int, c_ptrTo(gcm_delta_z), c_ptrTo(gcm_zw), c_ptrTo(gcm_zt), active_level_cnt: c_int);
    ...
    proc setSurfaceFluxForcingValues(const ref nz, const ref nt, ref tracerArray, const ref salinity, const ref temperature, const ref uwind, const ref vwind, const ref pressure, const ref pco2air, const ref pco2air_alt, const ref dust, const ref fe, const ref nox, const ref nhy) {
      set_surface_flux_forcing_values(
        this, nz: c_int, nt: c_int, 
        c_ptrTo(tracerArray), 
        salinity: c_double, temperature: c_double, uwind: c_double, vwind: c_double, 
        pressure: c_double, pco2air: c_double, pco2air_alt: c_double, dust: c_double, 
        fe: c_double, nox: c_double, nhy: c_double);
    }
  ...
  extern proc init_marbl_instance(const ref marblWrapper: marbl_interop_type, const ref gcm_num_levels: c_int, const ref gcm_num_PAR_subcols: c_int, const ref gcm_num_elements_surface_flux: c_int, 
    gcm_delta_z: c_ptr(c_double), gcm_zw: c_ptr(c_double), gcm_zt: c_ptr(c_double), const ref active_level_cnt: c_int);
...
//
 ...
 // writeln("Reading scalar values...");
  ...
  const dist = new blockDist(boundingBox=tracerArrayDomain);
  const distributedDomain = dist.createDomain(tracerArrayDomain);
  //writeln("Tracer domain: ", tracerDomain);
  //writeln("Tracer array domain: ", tracerArrayDomain);

  var tracerArray: [distributedDomain] c_double;
  ...
  forall (c,t,z) in distributedDomain {
  ...
  // writeln("Reading scalar values...");
    ...
    forall colIdx_ in myTracerDomain.dim[0] {
      ...
      writeln("computing for column index ", colIdx);
      var columnTracers: [1..nt, 1..zt] c_double = myTracerArray[colIdx,..,..];

      var marblWrapper: marbl_interop_type;
      marblWrapper.initMarblInstance(zt, 1, 1, delta_z, zw, ztCol, activeLevelCount[colIdx]);
      marblWrapper.setSurfaceFluxForcingValues(
        zt, nt, columnTracers, salinity[colIdx:int, 1], 
